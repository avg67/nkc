...
bool highscore::Load(void) {
    ...
    uint8_t result = jd_fillfcb(&myfcb,this->FileName.c_str());
    if (result==0) {
...
bool highscore::Append(const std::string name, const time_t time, const uint16_t duration, const uint16_t level)
{
    bool success=false;
    highscore_t* p_hs = (highscore_t*)this->p_buffer;
    if (p_hs->nr_entries < ARRAY_SIZE(p_hs->hs)) {
        ...
        p_hs->nr_entries++;
        strncpy(p_hs->hs[i].name,name.c_str(),MAX_NAME_LENGTH);
...
uint8_t highscore::InsertSorted(const std::string name, const time_t time, const uint16_t duration, const uint16_t level)
{
    ...
    strncpy(p_hs->hs[found_idx].name,name.c_str(),MAX_NAME_LENGTH);
...
/*
bool hs_compare (const highscore_entry_t * i, const highscore_entry_t * j) {
    return (i->duration < j->duration);
}

void highscore::sort(const uint16_t filter, std::vector<highscore_entry_t *>& idx_array) {
    // Index for sort
    //highscore_entry_t * p_hs[MAX_ENTRIES];
    highscore_t * const p_hs = (highscore_t * const)this->p_buffer;

    // push all relevant entries to array
    for (uint16_t i=0;i < p_hs->nr_entries;i++) {
        if ((filter == UINT16_MAX) || (p_hs->hs[i].level == filter)) {
            idx_array.push_back(&p_hs->hs[i]);
        }
    }
    // and sort array with duration as compare value
    std::sort(idx_array.begin(), idx_array.end(), hs_compare);
}*/
...
void highscore::Display(const uint16_t filter, const uint8_t hlEntry) {
   ...
   /*std::vector<highscore_entry_t *> idx_array;
   sort(filter,idx_array);
   // Display max first 10 HS items and highlight currently added one with green
    for(auto i = idx_array.begin(); i != idx_array.end(); ++i) {
        const highscore_entry_t *p_hs = *i;
        iprintf("%04u %04u %s\r\n",p_hs->duration, p_hs->level, p_hs->name);
    }*/
