...
void dhcp_init (void)
{
    //Port in Anwendungstabelle eintragen f�r eingehende DHCP Daten!
    ...
    switch (dhcp_state)
    {
      ...
      case DHCP_STATE_DISCOVER_SENT:
        if (gp_timer == 0)
        {
          dhcp_state = DHCP_STATE_IDLE;
          timeout_cnt++;
          DEBUG("State: DHCP_STATE_DISCOVER_SENT Timeout\r\n");
        }
      break;
      ...
      case DHCP_STATE_ACK_RCVD:
        DHCP_DEBUG("LEASE %2x%2x%2x%2x\r\n", cache.lease.bytes[0],cache.lease.bytes[1],cache.lease.bytes[2],cache.lease.bytes[3]);
        ...
        dhcp_lease = cache.lease.ipl;

        netmask.ipl       = cache.netmask.ipl;
        router_ip.ipl     = cache.router_ip.ipl;
        //Broadcast-Adresse berechnen
        broadcast_ip.ipl = ((myip.ipl & netmask.ipl) | (~netmask.ipl));
        #if USE_DNS
        dns_server_ip.ipl = cache.dns1_ip.ipl;
        #endif
...
void dhcp_message (unsigned char type)
{
  ...
  *options++       = 4;     // len = 4
  *options++       = myip.bytes[0];
  *options++       = myip.bytes[1];
  *options++       = myip.bytes[2];
  *options++       = myip.bytes[3];
  ...
  switch (type)
  {
    ...
    case DHCPREQUEST:
      ...
      *options++       = 4;     // len = 4
      *options++       = cache.serv_id.bytes[0];
      *options++       = cache.serv_id.bytes[1];
      *options++       = cache.serv_id.bytes[2];
      *options++       = cache.serv_id.bytes[3];
...
inline void get4bytes (unsigned char *source, unsigned char *target)
{
  ...
  do
  {
    switch (msg[ix])
    {
      ...
      case 1: //Netmask
        ...
        {
          ix++;
          get4bytes(&msg[ix], &c->netmask.bytes[0]);
          //*((uint32_t*)&msg[ix]) = *((uint32_t*)&c->netmask.bytes[0]);
          ix += 4;
        }
        else
        {
      ...
      case 3: //router (gateway IP)
        ...
        {
          ix++;
          get4bytes(&msg[ix], &c->router_ip.bytes[0]);
          ix += 4;
        }
        else
        {
      ...
      case 6: //dns len = n * 4
        ...
        {
          ix++;
          get4bytes(&msg[ix], &c->dns1_ip.bytes[0]);
          ix += 4;
        }
        else
        if ( msg[ix] == 8 )
        {
          ...
          get4bytes(&msg[ix], &c->dns1_ip.bytes[0]);
          ix += 4;
          get4bytes(&msg[ix], &c->dns2_ip.bytes[0]);
          ix += 4;
        }
        else
        {
      ...
      case 51: //lease time
        ...
        {
          ix++;
          get4bytes(&msg[ix], &c->lease.bytes[0]);
          ix += 4;
        }
        else
        {
      ...
      case 54: //Server identifier
        ...
        {
          ix++;
          get4bytes(&msg[ix], &c->serv_id.bytes[0]);
          ix += 4;
        }
        else
        {
...
void dhcp_get (void)
{
  ...
  switch (cache.type)
  {
    case DHCPOFFER:
      ...
      myip.ipl = (*((unsigned long*)&msg->yiaddr[0]));
      //Broadcast-Adresse berechnen
      //broadcast_ip.ipl = (((*((unsigned long*)&myip[0])) & (*((unsigned long*)&netmask[0]))) | (~(*((unsigned long*)&netmask[0]))));
      broadcast_ip.ipl = ((myip.ipl & netmask.ipl) | (~netmask.ipl));
