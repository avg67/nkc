...
static volatile unsigned short enc_next_packet_ptr = 0;
...
unsigned char enc_read_reg( unsigned char reg )
{
	unsigned char value;
	unsigned char addr = reg & ENC_REG_ADDR_MASK;
...
static void enc_write_phyreg( unsigned char phyreg, unsigned short value )
{
	enc_write_reg( ENC_REG_MIREGADR, phyreg );
...
static void enc_read_buf( unsigned char *buf, unsigned short len )
{
	enc_select();
...
static void enc_write_buf( unsigned char *buf, unsigned short len )
{
	enc_select();
...
void enc_send_packet( unsigned short len, unsigned char *buf )
{
	unsigned char ctrl = 0;
	unsigned short ms = 100;
  ...
  //enc_setbits_reg( ENC_REG_EIE, (1<<ENC_BIT_TXIE) | (1<<ENC_BIT_INTIE) );
	// start transmission by setting the TXRTS bit
	enc_setbits_reg( ENC_REG_ECON1, (1<<ENC_BIT_TXRTS) );
	//getchar();
}

unsigned short enc_receive_packet( unsigned short bufsize, unsigned char *buf )
{
	...
	ENC_DEBUG("enc_receive: EPKTCNT=%i ", (int) u);
	if( u == 0 ) {
	...
	enc_read_buf( (unsigned char*)rxheader, sizeof(rxheader) );
	//enc_next_packet_ptr  =             rxheader[0];
	//enc_next_packet_ptr |= ((unsigned)(rxheader[1])) << 8;
	enc_next_packet_ptr  = to_bendian16(rxheader[0]);
	//len                  =             rxheader[2];
	//len                 |= ((unsigned)(rxheader[3])) << 8;
	len                  = to_bendian16(rxheader[1]);
	//status               =             rxheader[4];
	//status              |= ((unsigned)(rxheader[5])) << 8;
	status               = to_bendian16(rxheader[2]);
