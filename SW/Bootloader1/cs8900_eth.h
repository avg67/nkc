...
#define RBUF_HEAD_LEN        0x0004              // Length of this header
    ...
    static inline void Write_Frame_long(const uint32_t data)
    {
        Write_Frame_word(data >> 16u);
        Write_Frame_word(data & 0xffffu);
    }

    // reads a word from the Frame Register High-Low-Order! Req. for Status and Length
    static inline uint16_t Read_FrameHL_word(void)
    {
    ...
    static inline uint16_t Read_Frame_word(void)
    {
    ...
    static inline uint32_t Read_Frame_long(void)
    {
	...
	*************************************************************/
    ...
    void request_send(const uint16_t frame_size);
