...
static inline __attribute__((always_inline)) void gp_setcolor(const uint8_t color) {
  asm volatile(
    "# asm"                      "\n\t" \
    "moveb %0,%%d0"              "\n\t" \
    "moveq %1,%%d7"              "\n\t" \
...
static inline __attribute__((always_inline)) uint32_t jd_getversi(void) {
  ...
  //register uint32_t jados_version __asm__("%d0");
  asm volatile(
    "# asm"                      "\n\t" \
    "movem.l %%d7/%%a5-%%a6,-(%%sp)"  "\n\t" \
    "moveq %1,%%d7"              "\n\t" \
