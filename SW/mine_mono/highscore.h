...
#define MAX_FILE_NAME_LENGTH (16u)
...
class highscore
{
    ...
    public:
        // class constructor
        highscore(const char * p_FileName);
        ~highscore();
        bool Load(void);
        bool Save(void);
        bool Append(const char* const p_name, const time_t time, const uint16_t duration, const uint16_t level);
        uint8_t InsertSorted(const char* const p_name, const time_t time, const uint16_t duration, const uint16_t level);
